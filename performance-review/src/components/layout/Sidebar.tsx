...
const iconMap = {
  ...
  flag: { outline: FlagIcon, solid: FlagSolidIcon },
...
function SidebarContent({ navigationItems, isActive, getIcon, onItemClick }: SidebarContentProps) {
  ...
  const handleLogout = async () => {
    try {
      await logout();
      if (onItemClick) onItemClick(); // Close mobile menu if open
    ...
    <div className="flex-1 flex flex-col pt-5 pb-4 overflow-y-auto">
      ...
      <div className="flex-shrink-0 px-2 pb-2">
        <button
          ...
          className="group flex items-center w-full px-2 py-2 text-sm font-medium rounded-md text-gray-600 hover:bg-red-50 hover:text-red-700 transition-colors"
        >
          <ArrowRightOnRectangleIcon className="mr-3 flex-shrink-0 h-5 w-5 text-gray-400 group-hover:text-red-500" />
          Logout
        </button>
      </div>
